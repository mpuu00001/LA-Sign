...
from config import rgb_dirs, pose_dirs, rgb_dirs_gpu, pose_dirs_gpu
...
class S2T_Dataset(Base_Dataset):
    def __init__(self, path, args, phase):
        ...
        if self.args.dataset == "CSL_Daily" or self.args.dataset == "OpenASL":
            if args.from_gpu:
                self.pose_dir = pose_dirs_gpu[args.dataset]
                self.rgb_dir = rgb_dirs_gpu[args.dataset]
            else:
                self.pose_dir = pose_dirs[args.dataset]
                self.rgb_dir = rgb_dirs[args.dataset]
                
        elif "WLASL" in self.args.dataset or self.args.dataset == "How2sign":
            if args.from_gpu:
                self.pose_dir = os.path.join(pose_dirs_gpu[args.dataset], phase)
                self.rgb_dir = os.path.join(rgb_dirs_gpu[args.dataset], phase)
            else:
